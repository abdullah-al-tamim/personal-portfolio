    ...
    <div className="w-[70px] h-full fixed left-0 top-0 flex flex-col justify-between border-r border-gray-200 px-4 py-10 z-10">
        ...
        <div className="flex flex-col gap-y-3 sm:gap-y-2 ">
            {navbarData.map((item, i) => (
                ...
                <span className={`text-[10px] tracking-wide -translate-x-2 opacity-0 group-hover:translate-x-0 group-hover:opacity-100 transition-all duration-300 text-center dark:text-white ${item.id === id && "-translate-x-0 opacity-100" }`}>{item.name}</span>
            </a>
            ))}
